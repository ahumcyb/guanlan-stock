...
from .artifacts import STRATEGIES,PREVIOUS_STRATEGIES,valid_strategy_group,GENERATION,CODE,MAX_REPORT,MAX_CHART,MAX_DETAIL,checked_file,atomic_json,preserved_legacy_generations,WATCH_STATES,priority_chart_codes
...
def extract(bundle,destination):
    ...
    if not valid_strategy_group(present):raise ValueError('Research strategy group invalid')
    ...
    if not chart_codes.issubset(codes):raise ValueError('Unexpected charts')
    if required and not required.issubset(chart_codes):raise ValueError('Missing priority charts')
    if extended.exists():
