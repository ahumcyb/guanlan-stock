...
def validate_details(folder,stocks):
    """Every short-list row must have one matching, bounded full detail."""
    if not folder.is_dir() or folder.is_symlink():raise ValueError('Stock details missing')
    expected={stock['ts_code']+'.json' for stock in stocks}
    actual={item.name for item in folder.iterdir()}
    if actual!=expected:raise ValueError('Stock detail set differs from report')
    for stock in stocks:
        path=checked_file(folder,folder/(stock['ts_code']+'.json'),MAX_DETAIL)
        full=json.loads(path.read_text())
        if not isinstance(full,dict) or any(full.get(key)!=value for key,value in stock.items()):
            raise ValueError('Stock detail differs from report')
...
def _copy_shared_charts(stage,chart_root,extended_root,codes,asof,data_revision):
    """Copy the requested shared charts; a missing source aborts publication."""
...
def publish(outputs,root,data_revision,strategies=None,charts_root=None,chart_codes=None):
    ...
    try:
        ...
        for strategy in strategies:
            ...
            if any(any(type(stock.get(flag)) is not bool for flag in ['trend_ok','strength_ok','pullback_ok','volume_ok','turn_ok']) for stock in report['stocks']):
                raise ValueError('Legacy stock condition flags missing')
            report=dict(report)
            ...
            validate_details(details,report['stocks'])
            shutil.copytree(details,destination/'details')
        ...
        copied=_copy_shared_charts(stage,shared_charts,extended if extended and extended.is_dir() else None,wanted,asof,data_revision)
        if copied!=len(wanted):raise ValueError('Shared charts incomplete')
