...
class Service:
    ...
    def content_revisions(self,reports):
        ...
        daily_digest=''
        try:
            latest=self.daily.public().get('latest') or {}
            daily_digest=str(latest.get('evidence_sha256') or latest.get('date') or '')
        ...
        return dict(reports=generation,daily=daily_digest,realtime=realtime_digest,realtime_history=history_digest)
