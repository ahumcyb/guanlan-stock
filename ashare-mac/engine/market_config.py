...
def market_configuration():
    value=dict(schema_version=1,provider='promax',base_url='http://127.0.0.1:8080',workers=24,primary_provider=None)
...
def market_provider_name():
    ...
    if config['primary_provider'] and config['primary_provider']!=config['provider']:
        return 'Mac：'+names[config['primary_provider']]+'；服务器备用：'+names[config['provider']]
