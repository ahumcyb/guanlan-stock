...
MAX_AGE = 180
BOTTOM_RULE_VERSION = 2
BOTTOM_MIN_VOLUME = 2.5
...
def bottom_volume_screen(features,quotes,now,expected_previous_date):
    ...
    for q in quotes:
        ...
        try:
            ...
            quote_at=finite(q['quote_at']);change=finite((close-previous)/previous*100);vwap=finite(q['amount'],True)/volume
            ...
            if multiple<BOTTOM_MIN_VOLUME-1e-8 or close<=previous or not -1e-8<=distance<=.10+1e-8:continue
            finite(vwap,True);elapsed=trading_minutes(now)
            matches.append(dict(strategy='bottom_volume',ts_code=code,name=str(f['name'])[:30],price=round(close,4),change=round(change,6),
                ...
                checks=['现价在近60日最低价上方0%–10%','累计成交量≥前5日全天均量2.5倍','最新价高于前收，当日上涨'],
    ...
    return dict(rule_version=BOTTOM_RULE_VERSION,status='ready' if matches else 'empty',lookback=60,matched_count=len(matches),candidates=matches[:10],
