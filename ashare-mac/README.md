...
- **数据管理**：选择本地行情目录，通过 ProMax 补齐最近 120 个市场交易日的日线、复权因子和涨跌停价，并更新交易日历及股票列表。
- **导出 CSV**：导出当前筛选或观察列表。代码与名称可直接查阅。

“重新选股”只读本地数据；“更新数据”会联网拉取缺口并在完成后重新选股。进度显示在窗口底部，更新/计算过程中可取消，原先的有效报告保留。
...
软件可用与策略获利能力是两件事。首版回踩模型在初始可结算历史样本中没有跑赢简单流动性参照，因此加入固定的流动性趋势模型作对照；没有网格搜索参数，也没有把历史结果包装成“已验证优秀”。**这两套模型仍是探索性研究，应以实际“历史检验”页面显示的结果为准。** 评分不是获利概率。
