...
Mac 1.10.0（13）新增[共享交互 K 线](docs/KLINE_UPGRADE.md)：日/周/月周期、十字线查价、平移缩放、独立量能和 MACD/RSI 副图、连续价/原始价、大图与历史提醒定位。当前历史最多412个交易日，新接口可承载500根；旧120根接口继续兼容。部署与验证见[验收记录](docs/KLINE_VERIFICATION_20260912.md)。

此前版本已在收盘总结加入[昨日实时策略的今日收益](docs/REALTIME_DAILY_VERIFICATION_20260910.md)：分列昨收至今收、提醒价至今收的复权观察表现，保留来源轮次、逐股价格及缺数状态。[达塔批量初筛＋D6逐股复核](docs/D101_BATCH.md)、日线更新及服务器离线接管继续按原配置运行；[底部放量](docs/BOTTOM_VOLUME.md) 仍为 14:30 检查至少 2.5 倍且当日上涨。
...
按用户后续要求加入 [60 日风险调整动量](docs/MOMENTUM_60.md)。它在此前八个新候选的选择期中胜率最高（48.69%），但开发期亏损，尚未通过完整验证；两端说明保留分阶段资金账本结果。原始动量比值取 Top5，当前筛选和动态事件历史与此前封存研究的口径分别说明。

2026-09-08 按用户要求，首页第四项改为 [左侧低吸](docs/LEFT_REBOUND.md)，使用最新完整日线筛选超跌、缩量和抛压收敛形态。首次历史检验没有显示盈利优势，作为研究观察策略呈现；原动量历史仍可读取。收盘总结已改为“上一交易日已保存精选 → 本日复权观察涨跌”，详见 [结算口径](docs/DAILY_SUMMARY.md)。
