    ...
    func request(_ action:String,runtime:Runtime,values:[String:Any]=[:]) async {
        ...
        do {
            ...
            if action=="state" {
                ...
                if state?.latest?.generatedAt != value.latest?.generatedAt || state?.running != value.running || state?.events.map(\.id) != value.events.map(\.id) {
                    state=value
                }
                ...
                if history.map(\.slot) != records.runs.map(\.slot) { history=records.runs }
