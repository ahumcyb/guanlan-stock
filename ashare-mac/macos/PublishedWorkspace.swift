        ...
        while !Task.isCancelled {
            if let api=publishedAPI {
                do {
                    ...
                    if dailyState==nil || revisions?.daily != lastContentRevisions?.daily,let data=try? await api.request("/v2/daily",limit:128*1024),let value=try? mobileDecoder().decode(DailyState.self,from:data),(try? value.validate()) != nil {
                        dailyState=value
                    }
                    if realtimeState==nil || revisions?.realtime != lastContentRevisions?.realtime,let data=try? await api.request("/v1/realtime",limit:2*1024*1024),let value=try? mobileDecoder().decode(RealtimeState.self,from:data) {
                        realtimeState=value
                    }
                    if dailyJev==nil || revisions?.dailyJev != lastContentRevisions?.dailyJev { await refreshDailyJev() }
