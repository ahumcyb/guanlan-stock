...
extension AppStore {
    ...
    func synchronizePublished() async {
        ...
        do {
            let state=try mobileDecoder().decode(ServerStatus.self,from:await api.request("/v1/status",limit:65536));serverStatus=state
            progress="正在同步已发布的四套策略…"
    ...
    func watchPublishedState() async {
        while !Task.isCancelled {
            if let api=publishedAPI {
                do {
                    serverStatus=try mobileDecoder().decode(ServerStatus.self,from:await api.request("/v1/status",limit:65536))
                ...
                if let data=try? await api.request("/v1/daily",limit:128*1024),let value=try? mobileDecoder().decode(DailyState.self,from:data), (try? value.validate()) != nil { dailyState=value }
