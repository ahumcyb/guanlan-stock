...
class MobileAPITests(unittest.TestCase):
    ...
    def test_missing_or_mismatched_stock_detail_blocks_publication(self):
        outputs=self.outputs();path=outputs/'leaders/20260905T120000-abcdef/details/000001.SZ.json'
        path.unlink()
        with self.assertRaisesRegex(ValueError,'Stock detail set'):
            publish(outputs,self.root,'20260904-aaaaaaaaaaaaaaaa')
        atomic_json(path,{'ts_code':'000001.SZ','close':99})
        with self.assertRaisesRegex(ValueError,'Stock detail differs'):
            publish(outputs,self.root,'20260904-aaaaaaaaaaaaaaaa')
    ...
    def test_published_snapshots_are_pinned_and_private_paths_removed(self):
        ...
        self.assertEqual(set(report['stocks'][0]),{'ts_code','name','industry','trade_date','close','change','score','state','rank','eligible','stale','adjusted','limit_available','trend_ok','strength_ok','pullback_ok','volume_ok','turn_ok'})
