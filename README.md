...
2026-09-08 更新：Mac 1.7.0（8）已安装，四策略读取统一发布结果，自选逐条同步，通知可回到固定轮次，首页与日报更聚焦变化。新增交易日14:30「底部放量3倍」，通知正文含名称、代码和倍数。iOS 1.6.0（8）代码和构建已准备，按用户要求暂未安装手机。服务器 257 项测试通过；当天实时失败的 1 KiB 网关限制已定位并修复。
