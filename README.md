...
2026-09-14：已恢复并发布当日收盘行情，5530只日线、同日复权因子与涨跌停价完成覆盖核验，四策略和413根扩展K线已回读。ProMax分页、分批、限流及等价日期区间重试已修复；328项测试在Mac和服务器通过。名录仍沿用有效缓存，今日量价总结可读，AI解读暂不可用。详见[排查记录](ashare-mac/docs/MARKET_RECOVERY_20260914.md)。

2026-09-12：新增日/周/月交互 K 线、大图、缩放平移、MA/MACD/RSI、原始价切换和提醒/日报定位。Mac 1.10.0（13）已安装；服务器新接口已发布，原生回读412根日线，旧120根接口保持兼容。Mac与服务器各309项Python测试、完整原生测试通过。iOS 1.9.0（13）已通过 Wi-Fi 安装到配对手机，设备回读版本一致并成功启动；签名已续至9月19日。详见[本次验收](ashare-mac/docs/KLINE_VERIFICATION_20260912.md)。
