...
面向 A 股 1–5 个交易日盘后研究的 macOS 与 iPhone 原生应用。SwiftUI 界面、Python 研究引擎、ProMax 行情更新，Mac 优先计算并上传结果，Mac 失联时服务器使用同一引擎接管。
...
两套策略为“流动性趋势”和“缩量回踩转强”，均属探索性研究，未完成独立样本外与模拟实盘验证。匹配分不是获利概率，历史事件收益不能视为实盘组合收益。软件不接自动下单。
