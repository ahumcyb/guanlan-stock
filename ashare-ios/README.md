...
iOS 1.7.1（11）在实时页面区分“批量覆盖”和“D6复核”数量。Mac 主节点支持[达塔批量初筛＋D6逐股复核](../ashare-mac/docs/D101_BATCH.md)，服务器离线接管保留原行情备用；手机读取统一发布结果。底部放量仍为 **至少 2.5 倍且当日上涨**。批量采集由 Mac 执行，不依赖手机运行。
