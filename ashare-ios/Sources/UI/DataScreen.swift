                ...
                VStack(alignment:.leading,spacing:16) {
                    ...
                    ResearchCard {
                        VStack(alignment:.leading,spacing:14) {
                            ...
                            Text("同步读取已发布结果；重新选股使用已有行情。更新会补齐必要日线及配套数据，再计算两套策略。").font(.caption).foregroundStyle(.secondary).lineSpacing(4)
                        }
                    }
                    if let report=store.report {
                        ResearchCard {
                            VStack(alignment:.leading,spacing:16) {
                                ...
                                Text("两套策略、价格和 K 线绑定同一数据版本。已下载的报告和最近查看的 K 线可离线使用。").font(.caption).foregroundStyle(.secondary).lineSpacing(4)
                            }
                        }
                    }
                    ResearchCard {
