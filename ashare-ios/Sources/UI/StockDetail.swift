...
struct MobileStockDetail:View {
    ...
    var body:some View {
        ScrollView {
            VStack(alignment:.leading,spacing:16) {
                ...
                ResearchCard {
                    if let data=chart.data { MobileCandleChart(data:data,name:shown.name,signalDate:shown.tradeDate,signalLabel:shown.state=="入选" ? "本次入选":"观察日",reference:shown.support,invalidation:shown.invalidation) }
    ...
    private func load() async {
        detail=try? await store.stockDetail(manifest,code:stock.id)
