...
struct JevStockView:View {
    ...
    var body:some View {
        ...
        VStack(alignment:.leading,spacing:8) {
            ...
            Text("行情："+realtimeDate(review.quoteAt)+" · "+String(format:"%.2f",review.price)).font(.caption).foregroundStyle(.secondary)
            Text(review.isExpired ? "基于原始快照的回看判断，不适用于当前买入。":review.explanation).font(.subheadline)
            Text("模型原始分类："+(["buy":"可考虑买入","watch":"观望","avoid":"暂不买"][review.modelChoice] ?? "未完成")+String(format:" · 置信度 %.0f%%（非盈利概率）",review.confidence*100)).font(.caption).foregroundStyle(.secondary)
...
struct JevReviewView:View {
    ...
    var body:some View {
        VStack(alignment:.leading,spacing:10) {
            ...
            ForEach(review.rows) { row in JevStockView(review:row);Divider() }
        }
    }
}
