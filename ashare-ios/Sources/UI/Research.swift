                    ...
                    if let report=store.report {
                        ...
                        ResearchCard {
                            VStack(alignment:.leading,spacing:14) {
                                ...
                                ForEach(report.backtest.monthly) { month in HStack { Text(month.month).font(.subheadline.monospaced());Spacer();Text("\(month.count) 次").font(.caption).foregroundStyle(.secondary);Text(percent(month.mean,signed:true)).font(.subheadline.monospacedDigit()).foregroundStyle(MobileTheme.change(month.mean ?? 0)).frame(width:86,alignment:.trailing) } }
                            }
                        }
                        ResearchCard { VStack(alignment:.leading,spacing:12) { Text("策略和验证边界").font(.headline);Text("流动性趋势：成交额前20%、20日强度前50%，收盘 > MA20 > MA60，短期延续且不过热。\n\n缩量回踩：20日强度前35%，距离近10日高点回撤1%–10%，近3日成交额收缩后收盘转强。\n\n基础池要求沪深非ST、充足日线、日均成交额≥1亿元、价格≥3元、ATR≤6%；市场宽度至少40%，每行业最多2只，共10只候选。").font(.subheadline).foregroundStyle(.secondary).lineSpacing(4);ForEach(report.warnings,id:\.self) { Label($0,systemImage:"info.circle").font(.caption).foregroundStyle(MobileTheme.amber) } } }
