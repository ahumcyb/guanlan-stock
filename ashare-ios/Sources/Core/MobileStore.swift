...
@MainActor final class MobileStore:ObservableObject {
        ...
        if startAutomatically { do {
            ...
            Task {
                if let saved=try? await loadVerifiedSnapshot(cache:cacheRoot,strategy:strategyChoice) {
                    ...
                    if let m=saved.manifest { dailyJev=cachedDailyJev(cacheRoot.root,m) }
                }
            }
    ...
    func changeStrategy(_ value:String) {
        ...
        Task {
            if let saved=try? await loadVerifiedSnapshot(cache:cache,strategy:value) {
                snapshot=saved
                if let m=saved.manifest { dailyJev=cachedDailyJev(cache.root,m) }
            } else { snapshot=nil }
            await synchronize()
        }
    }
    func synchronize() async {
        ...
        do {
            ...
            if let saved=try await loadVerifiedSnapshot(cache:cache,strategy:selected) {
                snapshot=saved
                guard let manifest=saved.manifest else { throw MobileFailure.invalidData }
    ...
    func watchJob() async {
        while !Task.isCancelled {
            ...
            let dailyChanged=revisions?.daily != lastRevisions?.daily
            let realtimeChanged=revisions?.realtime != lastRevisions?.realtime
            let historyChanged=revisions?.realtimeHistory != lastRevisions?.realtimeHistory
            if reportsChanged,!busy { await synchronize() }
            if dailyChanged { await refreshDaily();await refreshDailyJev() }
    ...
    func refreshRealtime() async {
        ...
        do {
            ...
            if !sameRealtime(realtime,value) {
                realtime=value
            }
            realtimeMessage=value.running ? "正在检查实时行情":"已同步实时提醒状态"
        } catch { realtimeMessage="实时服务暂未连接；请下拉刷新，已有研究结果仍可使用。" }
    }
    private func sameRealtime(_ lhs:RealtimeState?,_ rhs:RealtimeState)->Bool {
        guard let lhs else { return false }
        return lhs.running==rhs.running && lhs.executor==rhs.executor && lhs.macOnline==rhs.macOnline
            && lhs.serverOnline==rhs.serverOnline && lhs.settings.enabled==rhs.settings.enabled
            && lhs.latest?.slot==rhs.latest?.slot && lhs.latest?.generatedAt==rhs.latest?.generatedAt
            && lhs.lastScreen?.slot==rhs.lastScreen?.slot && lhs.lastScreen?.generatedAt==rhs.lastScreen?.generatedAt
            && lhs.lastBottom?.slot==rhs.lastBottom?.slot && lhs.lastFlow?.slot==rhs.lastFlow?.slot
