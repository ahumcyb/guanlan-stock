...
struct MobileManifest:Codable,Equatable {
    ...
    func validate() throws {
        guard schemaVersion==1,["leaders","pullback"].contains(strategy),
...
final class OfflineCache {
    ...
    func load(_ strategy:String) throws -> CachedSnapshot? {
        guard ["leaders","pullback"].contains(strategy) else { throw MobileFailure.invalidData }
    ...
    @discardableResult func save(_ data:Data,manifest:MobileManifest) throws -> CachedSnapshot {
        ...
        let protected=Set(["leaders","pullback"].compactMap { try? load($0)?.manifest }.map { reportURL($0).lastPathComponent })
