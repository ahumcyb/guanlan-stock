...
for filename in ['ChartData.swift','ChartMath.swift','ChartLoadState.swift','KlineDrawing.swift','KlineTouch.swift','KlineView.swift']:
    label='Shared '+filename
    reference=add(label,'PBXFileReference',lastKnownFileType='sourcecode.swift',name=label,path='../ashare-mac/macos/'+filename,sourceTree='<group>')
    references.append(reference);sources.append(add(label+' build','PBXBuildFile',fileRef=reference))
...
for name in ['Debug','Release']:
    ...
    settings={'PRODUCT_BUNDLE_IDENTIFIER':'local.guanlan.ios.bennie','PRODUCT_NAME':'Guanlan','SWIFT_VERSION':'5.0',
        ...
        'ASSETCATALOG_COMPILER_APPICON_NAME':'AppIcon','CURRENT_PROJECT_VERSION':'20','MARKETING_VERSION':'1.13.2',
